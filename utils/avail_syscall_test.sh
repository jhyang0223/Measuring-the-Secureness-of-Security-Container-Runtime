...
sudo docker run --runtime=runsc -i -t -h ${program} -v ${VOLUME_DIR}:${VOLUME_DIR} --cap-add SYS_PTRACE --name ${program} ${program} ./syscall_list.sh
...
do
    sudo docker run --runtime=runsc -i -t -h ${program} -v ${VOLUME_DIR}:${VOLUME_DIR} --cap-add SYS_PTRACE --name ${program} ${program} ./syscall_test_script_in_container.sh ${syscall}
    docker stop ${program}
    docker rm ${program}
done
