...
for ipcSyscall in "${ipcArray[@]}"; do
    for program in  $(ls -l /opt/ltp/testcases/kernel/syscalls/ipc/${ipcSyscall}/ | grep rwx | awk '{print $9}')
        do
            if [[ ${program} == "msgstress04" ]] ; then
                ...
                timeout -s 9 300s /opt/ltp/testcases/kernel/syscalls/ipc/${ipcSyscall}/${program}
            fi
    done
done
